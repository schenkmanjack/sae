...
class LogSparseFeatures:
    ...
    def analyze_sparse_features(self):
        ...
        for batch_idx, batch in enumerate(self.text_dataloader_static):
            ...
            with torch.no_grad():
                llm_model = torch.nn.DataParallel(self.llm_model)
                outputs = llm_model(**inputs)

            activation = self.mlp_activations[f'mlp_{self.target_layer}']
            
            # Reshape activations: (batch, seq_len, hidden_dim) → (batch * seq_len, hidden_dim)
            batch_size, seq_len, hidden_dim = activation.shape
            activation = activation.view(-1, hidden_dim)

            # Get the hidden activations from the sparse autoencoder
            sparse_autoencoder = torch.nn.DataParallel(self.sparse_autoencoder)
            _, hidden_activation = sparse_autoencoder(activation)  # (batch * seq_len, hidden_dim)
            ...
            for feature_idx in range(hidden_activation.size(1)):  # Iterate over sparse features
                feature_activations = hidden_activation[:, feature_idx]  # (batch * seq_len,)

                # Get current batch's top-k activations and their indices
                batch_topk_values, batch_topk_indices = torch.topk(feature_activations, k)

                # Compute corresponding batch index and token index
                batch_topk_batch_indices = batch_topk_indices // seq_len  # Compute batch index
                batch_topk_token_indices = batch_topk_indices % seq_len  # Compute token index

                # Store results
                if feature_idx not in global_topk_values:
                    global_topk_values[feature_idx] = batch_topk_values
                    global_topk_indices[feature_idx] = (batch_topk_batch_indices, batch_topk_token_indices)
                else:
                    ...
                    combined_batch_indices = torch.cat((global_topk_indices[feature_idx][0], batch_topk_batch_indices))
                    combined_token_indices = torch.cat((global_topk_indices[feature_idx][1], batch_topk_token_indices))
                    ...
                    global_topk_indices[feature_idx] = (
                        combined_batch_indices[new_topk_indices],
                        combined_token_indices[new_topk_indices]
                    )
        ...
        for feature_idx in global_topk_indices:
            batch_indices, token_indices = global_topk_indices[feature_idx]

            key_examples[str(feature_idx)] = [
                {
                    "context": self.tokenizer.decode(self.text_dataloader_static.dataset[idx][0][:token_idx.item() + 1]),  # All preceding tokens
                    "token": self.tokenizer.decode([self.text_dataloader_static.dataset[idx][0][token_idx.item()]])  # Single token
                }
                for idx, token_idx in zip(batch_indices, token_indices)
            ]

        # Save key examples to JSON
        with open(f"{self.results_dir}/features_{self.epoch}.json", "w") as f:
            json.dump(key_examples, f, indent=2)
...
#             key_examples[str(feature_idx)] = [
#                 {
#                     "context": self.texts[idx.item()],
#                     "token": self.tokenizer.decode(self.text_dataloader_static.dataset[idx][0][token_idx].item())
#                 }
#                 for idx, token_idx in zip(batch_indices, token_indices)
#             ]
